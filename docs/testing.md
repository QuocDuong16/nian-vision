...
| `nian-application` / media worker / desktop/UI (M11/current) | 16-session Live View capacity (initial M11 limit was four), duplicate/open reservation RAII, opaque loopback session capability, Host/Origin/path/method rejection, background keepalive expiry/reaper, per-camera worker-status isolation, bounded live retry/cancel, recording/live independence, suspend/hide/update cleanup and reconnect media remount |
...
| `nian-settings` / worker / `nian-application` / desktop/UI (Local Motion) | schema-v8 opt-in and ONVIF mutual exclusion; H.264/HEVC shared-packet luma decoder and scene-motion hysteresis; bounded transition backlog, separate durable/ACK cursors for lost acknowledgements, dead-worker lease reacquisition and reset-sequence detection, EventIndex-first ACK and replay dedupe; main pre-roll admission before substream detector launch; poisoned event-sink fails before index commit; diagnostic reads never spawn a worker or expose a source URL; UI distinguishes Desired On from detector monitoring/reconnect/unavailable, including an RPC-failure state. Hardware detection sensitivity, CPU/RAM and clip quality remain release acceptance, not CI fixture claims. |
| `nian-onvif` / `nian-index` / `nian-application` / desktop/UI (camera Person events) | Tapo-gated `PeopleDetector/IsPeople` classification vs `IsMotion`; EventIndex v3 atomic v1/v2 migrations preserving IDs, fingerprints and visibility; atomic source/aggregate inserts with fault-injected rollback and replay; separate Person start/end review rows associated to the same aggregate motion clip without duplicate recorder; explicit UI labels and type filter. Camera/firmware capability requires physical testing; no Nian AI claim. |
...
spoofing and service-authority mismatch. A staged-protocol regression feeds a complete-but-invalid configuration-options range and requires `PtzConfigurationOptionsProtocol`, so desktop errors can distinguish GetServices, media-profile/PTZ association and GetConfigurationOptions failures. A separate multi-space fixture proves a partial velocity-space candidate is ignored and cannot contaminate a later complete valid pan/tilt or zoom space. Adapter tests fingerprint only TP-Link/Tapo C200 metadata and prove the derived TCP 2020 `/onvif/service` candidate stays on the trusted device host. The client tests also assert bounded
...
`nian-index` Event Review tests cover all-camera and single-camera filtering, bounded receive-time ranges, deterministic receive-time/event-ID ordering, equal-timestamp tie breaking, page limits, keyset next cursors, no duplicate/skip across stable pages, malformed cursors, excessive limits, invalid ranges, empty results and retained-away rows. RC49+ no longer correlates Events to the manual Recording index. RC50 desktop Event-capture tests cover separate nearby MotionStarted→MotionEnded episodes, real five-second pre-roll windows, independent overlapping post-roll windows, immutable Event-ID aliases, multi-part continuation ordering, symlink rejection, idempotent/no-retarget metadata publication, per-part retention deletion and Event clip lookup isolation from the manual recording tree. Playback tests prove an open Event clip pins its episode identity until session close.
...
Desktop compilation/tests exercise the RC49 ownership split plus the RC50 burst-boundary refinement: Event capture owns a separate controller/output tree and no motion code can promote, start or stop the manual recording controller. Regression coverage also proves Desired Off during active motion and loss of authoritative ONVIF monitoring settle the active episode with bounded post-roll, revoke stale aggregate-motion state and cannot create a later five-minute continuation; healthy Desired/Polling monitoring leaves the episode untouched. The media-hardening tranche keeps that logical separation while Event/Recording/Focus share the camera main ingest and Event Desired retains only a bounded compressed pre-roll lease until an episode actually needs disk recording. Event history/query/get/recording-context/playback commands run blocking index/filesystem work through `spawn_blocking`; Event clip availability is resolved only through immutable Event aliases. Media-worker integration tests concatenate two real H.264/AAC Matroska fixtures into one playable event clip, prove segment timelines are cumulative rather than overlapped, round-trip the composed clip through `playback.prepare`, and enforce a hard requested duration bound at packet/mux boundaries. Long-motion UI coverage opens Clip 1/N and navigates Next/Previous parts. Retention coverage proves one part can be removed without retargeting or destroying sibling parts and playback coverage pins active Event episodes. Hide retains background Event monitoring/capture intent, while Suspend/Quit/Update settle owned workers according to lifecycle before Event restoration or process handoff; production native notification delivery remains isolated in an owned helper subprocess that is terminated and reaped on stop/deadline.

Frontend Event Review tests cover the RC50 one-burst/one-review-item projection, hidden raw MotionEnded rows on the default surface, the default 24-hour bounded query, camera filter propagation, selection-generation protection, stale-pagination rejection after filter changes, periodic-root-refresh invalidation, manual-root-refresh invalidation, exact frozen relative query bounds across later pages, and automatic selected-context refresh when a newly finalized Event clip becomes available. The screen uses semantic controls, finite ten-second single-flight/coalesced root refresh, generation-owned cursor pagination, zero seek into clips that already contain real pre-roll, and bounded Clip N/M Previous/Next navigation for continuous motion longer than one part. Settings tests verify the dedicated notification preference command rather than routing the toggle through storage settings.

Manual RC50 validation on an installed Windows/Linux desktop should verify OS notification presentation while the window is visible and hidden, no historical replay after restart/Resume, MotionStarted-only notification behavior and notification privacy text. For motion footage, enable Events with manual Recording Off, allow the compressed main-ingest pre-roll ring to warm, perform a visible movement before/through MotionStarted, and verify the Event clip contains about five seconds of real pre-roll plus five seconds of post-roll without continuous event-buffer disk writes while idle. Perform a second distinct movement during the first clip's post-roll and verify Event Review creates a second independent Motion event/clip rather than extending or merging the first clip; overlap between the two clips is expected. Then keep manual Recording continuously On while motion occurs and prove Timeline recording is neither stopped nor restarted while Event Review receives its own independent clip. Hold motion beyond five minutes and verify all bounded clip parts are exposed through Clip N/M navigation with no part exceeding five minutes. During a separate active motion, disable Events and verify only the bounded post-roll completes before capture stops; repeat with an ONVIF Event transport interruption and verify the clip settles instead of continuing indefinitely, then confirm recovery establishes a fresh synchronization baseline. Verify active Event playback survives retention housekeeping, old clips follow Event age/quota cleanup, automatic Event Review availability appears after finalization, and Event ingestion continues when OS notification display fails. Notification click activation is not claimed on the current Tauri desktop abstraction because it provides no click callback.
