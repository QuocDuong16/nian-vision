...
`nian-index` Event Review tests cover all-camera and single-camera filtering, bounded receive-time ranges, deterministic receive-time/event-ID ordering, equal-timestamp tie breaking, page limits, keyset next cursors, no duplicate/skip across stable pages, malformed cursors, excessive limits, invalid ranges, empty results and retained-away rows. RC49 no longer correlates Events to the manual Recording index. Desktop Event-capture tests cover aggregate episode construction, real five-second pre-roll windows, post-roll cancellation, immutable Event-ID aliases, multi-part continuation ordering, symlink rejection, idempotent/no-retarget metadata publication, per-part retention deletion and Event clip lookup isolation from the manual recording tree. Playback tests prove an open Event clip pins its episode identity until session close.
