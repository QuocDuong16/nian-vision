...
consecutive failure becomes an explicit per-tile error requiring manual Retry. Live presentation also offers Fit tile and Native pixels modes. Native pixels caps rendered CSS dimensions against decoded source dimensions divided by device-pixel ratio, so presentation upscaling is avoided when the tile has enough room. Optional diagnostics expose source dimensions, rendered device pixels, DPR and effective scale without altering the H.264 packet-copy/MSE transport.
...
namespace/DTD/entity hardening and authentication negotiation as M10. If ordinary PTZ/media service discovery is absent and bounded device metadata fingerprints a TP-Link/Tapo C200, a narrow adapter may try the fixed same-host TCP 2020 `/onvif/service` candidate through the same authority checks. PTZ setup protocol failures are split into GetServices, media-profile/PTZ association and GetConfigurationOptions stages; the adapter never invents velocity ranges. Pan/tilt is required;
...
Initial Event pairing for an already configured RTSP camera reuses its saved credential entirely inside the desktop/native credential boundary. The backend silently WS-Discovers devices, exact-host matches against the saved RTSP host, authenticates Events, and sends only `camera_id` across the frontend command boundary; explicit device selection and alternate credentials are used only when the saved credential is rejected or for replacement. The explicit session/device path remains generation-bound. Event service resolution prefers `GetServices` and falls back to standard `GetCapabilities(Events)` when Events are omitted there, then validates `GetEventProperties` without requiring ONVIF Media discovery. Generic capability/notification parsing accepts namespace-qualified standard `RuleEngine/CellMotionDetector/Motion` + `IsMotion`, `RuleEngine/MotionRegionDetector/Motion` + `State`, and `VideoSource/MotionAlarm` + `State`; missing Events service and present Events-without-compatible-motion-topic are reported as distinct typed failures. A bounded `GetDeviceInformation` fingerprint can select the TP-Link/Tapo C200 adapter only after the generic path is insufficient. That adapter may derive the fixed same-host TCP 2020 `/onvif/service`, probe PullPoint despite missing compatible topic advertisement, and normalize ONVIF-topic People/Smart Event/Line Cross states while retaining the same namespace, response and authority bounds. `EventController` performs a second exact-host check against the current RTSP camera and persisted Event binding before authenticated runtime traffic. `nian-onvif` independently validates Event/PullPoint URLs as HTTP(S), same-host, no userinfo/query/fragment, with redirects disabled and the existing proxy-free hardened SOAP client.
...
Recording context is resolved by the existing recording index using `CameraId + Event.received_time_utc`. Finalized segments with known duration are authoritative; directory scans and guessed paths are forbidden. The playback controller returns only safe session/context metadata, subtracts a fixed five-second playback pre-roll with saturating clamp at segment start, and reuses ordinary playback validation/pinning. RC47 also permits a small bounded forward-start grace when a motion-triggered recorder begins just after the persisted trigger, so the triggering Event can correlate with the first finalized clip. A missing segment, unconfigured recording storage or footage already removed by retention yields `available=false` while the Event remains valid.

RC47 adds a separate post-persistence motion-recording projection; Event ingestion itself remains recording-agnostic. Each newly committed transition carries the aggregate normalized camera motion state after that commit. A desktop-owned dispatcher tracks aggregate intent per camera and reconciles it under the same recording lifecycle gate used by manual controls. Aggregate active starts a transient motion-owned recorder only when no manual/external session owns the camera and persistent Recording Desired is Off. Aggregate idle starts a fixed five-second post-roll; a new active transition cancels that pending stop, while reactivation during `Stopping` is remembered and starts a fresh slot after the old worker is reaped. Motion ownership is transient and never writes persistent Recording Desired. Manual Start can promote a running motion-owned slot without restarting media, and later MotionEnded cannot stop that promoted/manual session. Motion sessions clamp the normal recording segment target to at most 300 seconds, so long movement rotates into consecutive files without restarting the recorder process at each five-minute boundary. Finalized-segment status drives a bounded recording-index refresh so Event Review discovers new footage; selected Event Review context rechecks when root refresh reports that a clip became available. RC47 does not implement a true pre-event encoded ring buffer: five-second Event playback pre-roll only applies when earlier footage already exists.
