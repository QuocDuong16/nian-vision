...
- **Events:** v1 stores normalized MotionStarted/MotionEnded history and RC49 maintains an independent packet-copy Event capture plane with about five seconds of real buffered pre-roll, five-second post-roll, immutable event-only clips and a hard maximum of five minutes per clip part. The Event buffer/clip plane is separate from manual Recording Desired, manual Start/Stop and Timeline. Event capture uses its own RTSP ingest while enabled, so camera connection limits still apply when Live View and manual Recording are also active. There is no Nian-side AI/person/object analysis, analytics configuration, event thumbnail/export workflow or cloud Event archive. Camera-provided Tapo People/Smart/LineCross signals remain compatibility inputs, not Nian AI.
