...
Current retry candidate: `1.0.0-rc.10` / `v1.0.0-rc.10`. RC1 through RC9 remain immutable. RC9 proved the shared semantic selected-Visual-Studio/MSVC authority on `windows-2022`, then failed deterministically in Windows preflight because `/usr/bin/make` was absent and bare `make` resolved to `/c/mingw64/bin/make`. RC10 provisions pinned official MSYS2 GNU make before preflight, sanitizes the shared FFmpeg MSYS PATH to keep selected MSVC first and `/usr/bin` second without MinGW/UCRT/CLANG toolchain bins, aggregates required tool diagnostics, preserves AWK/`CCDEP` and extraction fail-closed checks, and invalidates older Windows FFmpeg build-tool cache contracts.

## Pre-tag authority

- [ ] RC commit is on authoritative Forgejo default branch and every version surface is the same prerelease SemVer, currently `1.0.0-rc.10`.
...
- [ ] Create a new immutable prerelease tag exactly matching the RC source version, currently `v1.0.0-rc.10`. Never move, delete or reuse `v1.0.0-rc.1`, `v1.0.0-rc.2`, `v1.0.0-rc.3`, `v1.0.0-rc.4`, `v1.0.0-rc.5`, `v1.0.0-rc.6`, `v1.0.0-rc.7`, `v1.0.0-rc.8` or `v1.0.0-rc.9`; source/tag cross-pairing is forbidden.
