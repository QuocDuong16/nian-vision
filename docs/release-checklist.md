...
Current retry candidate: `1.0.0-rc.51` / `v1.0.0-rc.51`. RC1 through RC50 remain immutable. RC51 keeps RC50 recording/Event/PTZ/Live semantics unchanged while replacing the desktop presentation layer with a responsive interface system and lowering the supported Tauri minimum window to 720×520.

## Pre-tag authority

- [ ] RC commit is on authoritative Forgejo default branch and every version surface is the same prerelease SemVer, currently `1.0.0-rc.51`.
...
- [ ] Create a new immutable prerelease tag exactly matching the RC source version, currently `v1.0.0-rc.51`. Never move, delete or reuse any consumed tag `v1.0.0-rc.1` through `v1.0.0-rc.50`; source/tag cross-pairing is forbidden.
