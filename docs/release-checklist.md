...
Current retry candidate: `1.0.0-rc.52` / `v1.0.0-rc.52`. RC1 through RC51 remain immutable. RC52 continues the desktop UI/UX overhaul without changing the accepted recording/Event/PTZ/Live backend semantics, refining camera onboarding, custom controls, storage selection, dialog/layout behavior and Live View refresh handling.

## Pre-tag authority

- [ ] RC commit is on authoritative Forgejo default branch and every version surface is the same prerelease SemVer, currently `1.0.0-rc.52`.
...
- [ ] Create a new immutable prerelease tag exactly matching the RC source version, currently `v1.0.0-rc.52`. Never move, delete or reuse any consumed tag `v1.0.0-rc.1` through `v1.0.0-rc.51`; source/tag cross-pairing is forbidden.
