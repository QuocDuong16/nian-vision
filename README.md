...
**Status**: milestones **M0–M14 are accepted**. **M15 is the final v1 production-hardening/release milestone**. The current release-candidate source identity is Nian Vision 1.0.0-rc.6; final 1.0.0 source is created only after RC acceptance. RC1 through RC5 remain immutable historical release attempts: RC1 exposed the Linux Bash-shell mismatch, RC2 exposed container Git ownership trust, RC3 exposed Windows CRLF/fail-open native-command issues plus hosted-runner resource pressure, RC4 exposed a PowerShell parser error in the native-command failure message, and RC5 reached Linux AppImage bundling before failing with an opaque linuxdeploy error after the large Rust/Tauri compile. RC6 adds post-build Linux disk diagnostics, safe release-intermediate pruning, a second free-space guard and verbose AppImage diagnostics while preserving the accepted RC5 Windows/release behavior. v1 is not considered released until Forgejo CI, final review, tag-build validation and the documented Windows/Linux release-candidate checklist pass. M10 adds local ONVIF discovery and provisioning without
