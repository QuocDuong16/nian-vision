...
**Status**: milestones **M0–M14 are accepted**. **M15 is the final v1 production-hardening/release milestone**. The current release-candidate source identity is Nian Vision 1.0.0-rc.3; final 1.0.0 source is created only after RC acceptance. RC1 remains immutable historical evidence of a failed Linux release build caused by the container job using `/bin/sh` for Bash-only release commands. RC2 fixed the Bash shell but failed when Git rejected the mounted container checkout as unsafe. RC3 adds trust only for the exact `$GITHUB_WORKSPACE` before container-side Git operations, with no application-behavior change. v1 is not considered released until Forgejo CI, final review, tag-build validation and the documented Windows/Linux release-candidate checklist pass. M10 adds local ONVIF discovery and provisioning without
