...
media is a bounded rolling fragmented-MP4 window (500 ms target, 24 retained fragments,
...
network, authenticate transiently, inspect H.264/HEVC media profiles, resolve and verify the
...
duration enrichment, seekable packet-copy H.264/HEVC MP4 playback (AAC in MP4 or G.711 as PCM WAV) over
...
RTSP camera. M12 keeps PTZ optional and explicitly paired. M13 adds independent ONVIF PullPoint motion-event monitoring with persisted Desired state, bounded per-camera workers, normalized MotionStarted/MotionEnded history, and a dedicated rebuildable `<storage_root>/.nian/events.sqlite3` index. M14 adds a dedicated Event Review screen backed by the persisted Event index, bounded keyset pagination, explicit missing-footage states, and optional post-persistence local desktop MotionStarted notifications with bounded dispatch and per-camera rate limiting. RC49 supersedes the RC47 shared-recorder experiment with an independent motion Event-capture plane: a short rolling compressed-packet buffer provides real pre-roll, aggregate idle adds five seconds of post-roll, immutable event-only clips live under `.nian/event-clips`, long motion is split into bounded clip parts, and manual Recording Desired/Start/Stop plus Timeline remain completely separate. macOS distribution, ONVIF presets/talkback/vendor event dialects, video transcoding, Nian-side AI person/object recognition, cloud and remote streaming remain outside v1. HEVC recording/live and opt-in local scene motion are implemented, subject to runtime decoding limits.
...
* Tauri 2 desktop application (React/TypeScript/Vite UI) with managed M13 state:
  ...
  global RecordingState. A dedicated Live View screen lets the user select up to 16
...
* ONVIF onboarding is local and explicit. Discovery returns opaque Rust-owned
  ...
  Media fallback, and only H.264/HEVC video profiles are currently recordable. Final
...
* M14 Event Review reads normalized persisted `EventIndex` rows, while RC49+ resolves dedicated Event-owned clips rather than correlating against manual Timeline footage. RC50 projects one user-facing Motion event per aggregate MotionStarted→MotionEnded burst; raw MotionEnded transitions remain persisted for audit/debug but are hidden from the default review list. The default UI range is the last 24 hours; backend pages remain keyset-ordered by receive timestamp plus event ID, bounded to 31 days and 200 raw rows per request, and executed off the Tauri main thread. Camera/time filters never load the full retained index into React. Dedicated clips include real buffered pre-roll/post-roll and never change recording Desired state. Missing/retained-away/not-yet-finalized footage is an ordinary unavailable state, not an ingestion error. Optional desktop motion notifications default Off and are persisted independently in settings schema v6. Only a newly committed `MotionStarted` can enter the 32-item non-blocking notification queue; per-camera notifications are limited to one per 15 seconds, notifier failures cannot break Event monitoring, and historical rows are never replayed at startup. Hide leaves notifications running; Suspend/Resume/Quit/Update own bounded dispatcher teardown/restoration. Native notification content is limited to `Motion detected` plus the camera display name. The current Tauri desktop notification abstraction does not expose a click callback, so M14 deliberately does not fake notification deep-link activation.
...
* An isolated `nian-media-worker` process that talks FFmpeg through FFI:
  ...
  * `nian-media-worker run` serves a versioned NDJSON IPC protocol on
    ...
    one H.264/HEVC packet-copy live job per live worker. Live output is independently finalized
    ...
    host-validated finalized MKV and packet-copy remuxes supported H.264/HEVC video
    and AAC into MP4, or prepares G.711 PCM WAV beside video in the playback cache;
...
* Rust 1.98.1 (pinned in `rust-toolchain.toml`; mise users get it from
  `mise.toml` / `mise.lock`)
* Node 26.9.0 + standalone pnpm 12.4.2 (exact mise pins, mirrored by CI)
...
* RTSP (H.264/HEVC) — implemented at the media layer for recording and live view;
...
* ONVIF — implemented for local discovery/provisioning, optional M12 PTZ, and optional M13 PullPoint motion events. Device Management plus Media2/legacy Media resolves selected H.264/HEVC streams; recording/live still use RTSP through the existing media path. PTZ adds explicitly paired continuous pan/tilt and capability-gated zoom. Events add explicitly paired standard CellMotion monitoring with separate Enable/Disable Desired intent and normalized local history. Presets, audio talkback and video transcoding remain out of scope; HEVC playback/Live requires WebView/OS decoder support.

## ONVIF camera setup

Choose **Add camera → Discover ONVIF cameras** to scan the local network, select a
device, enter that camera's ONVIF credentials, choose a supported H.264 or H.265 profile and
...
* **No compatible profile**: select an H.264 or H.265 camera profile; other video codecs are not currently recordable. HEVC recording works by copying the original stream, while Live View and playback require a compatible OS/WebView decoder. Nian Vision does not transcode.
...
The v1 release targets are **Windows x86_64 (NSIS)** and **Linux x86_64 (AppImage)**. H.264 and H.265 RTSP are supported for packet-copy recording; there is no video transcoding and HEVC playback/Live depends on OS/WebView decoding. Accepted runtime limits are 8 simultaneous Recording sessions, 16 Live View sessions and 16 Event-monitoring sessions. The complete intentional boundary, including notification and ONVIF limitations, is in `docs/known-limitations.md`.
