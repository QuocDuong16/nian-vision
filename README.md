...
**Status**: milestones **M0–M14 are accepted**. **M15 is the final v1 production-hardening/release milestone**. The current release-candidate source identity is Nian Vision 1.0.0-rc.2; final 1.0.0 source is created only after RC acceptance. RC1 remains immutable historical evidence of a failed Linux release build caused by the container job using `/bin/sh` for Bash-only release commands; RC2 contains the release-infrastructure shell fix plus required version/documentation updates. v1 is not considered released until Forgejo CI, final review, tag-build validation and the documented Windows/Linux release-candidate checklist pass. M10 adds local ONVIF discovery and provisioning without
